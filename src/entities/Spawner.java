...
public class Spawner {
    ...
    public void tick() {
        ...
        if (elapsed >= spawnTimeEnemy1) startSpawn1 = true;
        if (elapsed >= spawnTimeEnemy2) startSpawn2 = true;
        if (elapsed >= spawnTimeEnemy3) startSpawn3 = true;

        if (startSpawn1) spawnEnemy1();
        if (startSpawn2) spawnEnemy2();
        if (startSpawn3) spawnEnemy3();
    }

    private void spawnEnemy1() {
        if (canSpawnEnemy1) {
            Game.enemies.add(new Enemy(random.nextInt(Game.WIDTH - 30), -20, 32, 48, null));
    ...
    private void spawnEnemy2() {
        if (canSpawnEnemy2) {
            Game.enemies.add(new Enemy2(random.nextInt(Game.WIDTH - 30), -20, 32, 48, null));
    ...
    private void spawnEnemy3() {
        if (canSpawnEnemy3) {
            Game.enemies.add(new Enemy3(random.nextInt(Game.WIDTH - 30), -20, 32, 48, null));
